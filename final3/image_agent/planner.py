...
class Planner(torch.nn.Module):
    def __init__(self):
        ...
        # BINARY CLASSIFICATION. PUCK IN IMAGE OR NOT IN IMAGE (PART 1)
        self.classifier2 = torch.nn.Linear(128, 1) # Number of channels is 128
    ...
    def forward(self, x):
        ...
        # BINARY CLASSIFICATION. PUCK IN IMAGE OR NOT IN IMAGE (PART 1)
        b = x39                 # torch.size([B, 128, 3, 4]) # H=3 and W=4 are small, which may give bad/decent training and validation accuracy but will be fast for training.
        b = b.mean([2,3])       # torch.Size([1, 128])         # Gets rid of H=300, and W=400
        b = self.classifier2(b) # torch,Size([1, 1]) 
        b = b[:,0]              # torch,Size([1]) 
        #print(b.size())
        ...
        # print(z.shape)
        puckPredImageCoordinates = (1 + spatial_argmax(x39.squeeze(1))) * torch.as_tensor([x.size(3) - 1, x.size(2) - 1]).float().to(x.device) #tensor([[0-128, 0-96]]) #torch.Size([1, 2])
        binaryClassification = b # torch.Size([B])
        return  puckPredImageCoordinates, binaryClassification
